 ...
 * then call @ref DarkHelp::predict() as often as necessary to determine what the images contain.
 * For example:
 * ~~~~
 ...
 * ~~~~
 *
 * Instead of calling @ref DarkHelp::annotate(), you can get the detection results and iterate through them:
 *
 * ~~~~
 * DarkHelp darkhelp("mynetwork.cfg", "mynetwork.weights", "mynetwork.names");
 *
 ...
 * ~~~~
 */
class DarkHelp final
{
	public:

		/// Forward declaration.  See further below in this file.
		class Config;
		...
		/// Vector of colours to use by @ref DarkHelp::annotate().  @see @ref DarkHelp::Config::annotation_colours  @see @ref DarkHelp::get_default_annotation_colours()
		 ...
		 * that the object belongs to that class.  @see @ref DarkHelp::PredictionResult::all_probabilities
		 */
		 ...
		 * caller every time @ref DarkHelp::predict() is called.  The most recent predictions are also stored in @ref DarkHelp::prediction_results.
		 */
		struct PredictionResult
		{
			 ...
			 * is used as a label when calling @ref DarkHelp::annotate().
		...
		/** A vector of predictions for the image analyzed by @ref DarkHelp::predict().  Each @ref DarkHelp::PredictionResult
		 * entry in the vector represents a different object in the image.
		 * @see @ref DarkHelp::PredictionResult
		 * @see @ref DarkHelp::prediction_results
		 * @see @ref DarkHelp::Config::sort_predictions
		 */
		...
		/// Constructor.  When using this constructor, the neural network remains uninitialized until @ref DarkHelp::init() is called.
		DarkHelp();

		/** Constructor.  This constructor automatically calls @ref DarkHelp::init() to load the neural network.  The neural
		 * network filenames within the @p cfg object must not be empty otherwise the call to @ref DarkHelp::init() will fail.
		 */
		DarkHelp(const Config & cfg);

		/** Constructor.  This constructor automatically calls @ref DarkHelp::init() with the given parameters.
		 *
		 * @note The order in which you pass the various filenames is @em not important if @p verify_files_first is set to
		 * @p true (the default value).  This is because @ref DarkHelp::init() will call @ref DarkHelp::verify_cfg_and_weights()
		 ...
		 */
		DarkHelp & init(const std::string & cfg_filename, const std::string & weights_filename, const std::string & names_filename = "", const bool verify_files_first = true, const EDriver driver = EDriver::kDarknet);

		/// Initialize ("load") the darknet neural network.  This uses the values within @ref DarkHelp::config.
		DarkHelp & init();

		/// The opposite of @ref DarkHelp::init().  This is automatically called by the destructor.
		void reset();

		/** Use the neural network to predict what is contained in this image.  This results in a call to either
		 * @ref DarkHelp::predict_internal() or @ref DarkHelp::predict_tile() depending on how
		 ...
		 * @ref DarkHelp::original_image will be set to this image.  If the image is larger or smaller than the dimensions of
		 ...
		 *
		 * @see @ref Tiling
		 * @see @ref DarkHelp::predict_tile()
		 * @see @ref DarkHelp::PredictionResult
		 * @see @ref DarkHelp::Config::enable_tiles
		 * @see @ref DarkHelp::Config::sort_predictions
		 * @see @ref DarkHelp::duration
		 */
...
#ifdef DARKHELP_CAN_INCLUDE_DARKNET
		/** Use the neural network to predict what is contained in this image.    This results in a call to either
		 * @ref DarkHelp::predict_internal() or @ref DarkHelp::predict_tile() depending on how @ref DarkHelp::Config::enable_tiles
		 * has been set.
		 * @param [in] mat A Darknet-style image object which has already been loaded and which needs to be analyzed.
		 * The member @ref DarkHelp::original_image will be set to this image.  If the image is larger or smaller than the
		 ...
		 * @param [in] new_threshold Which threshold to use.  If less than zero, the previous threshold will be applied.
		 * If >= 0, then @ref DarkHelp::threshold will be set to this new value.  The threshold must be either -1, or a value
		...
		/** Similar to @ref DarkHelp::predict(), but automatically breaks the images down into individual tiles if it is
		 * significantly larger than the network dimensions.  This is explained in details in @ref Tiling.
		 *
		 * @note The method @ref DarkHelp::predict() will @em automatically call @ref DarkHelp::predict_tile() if necessary
		 * <b>when @ref DarkHelp::Config::enable_tiles has been enabled.</b>  If you don't want to use image tiling, then
		 * @ref DarkHelp::Config::enable_tiles must be set to @p false (which is the default).
		 *
		 ...
		 * only returns the final results once each tile has been processed and the @ref DarkHelp::prediction_results "vectors"
		 * have been merged together.
		 *
		 * @image html mailboxes_2x2_tiles_detection.png
		 *
		 * @see @ref Tiling
		 * @see @ref DarkHelp::predict()
		 */
		PredictionResults predict_tile(cv::Mat mat, const float new_threshold = -1.0f);

		/** Takes the most recent @ref DarkHelp::prediction_results, and applies them to the most recent
		 * @ref DarkHelp::original_image.  The output annotated image is stored in @ref DarkHelp::annotated_image as well
		 ...
		 *
		 * Turning @em down the threshold in @ref DarkHelp::annotate() wont bring back predictions that were excluded due to
		 * a higher threshold originally used with @ref DarkHelp::predict().  Here is an example:
		 *
		 * ~~~~
		...
		/** Return @ref DarkHelp::duration as a text string which can then be added to the image during annotation.
		 * For example, this might return @p "912 microseconds" or @p "375 milliseconds".
		 * @see @ref DarkHelp::annotate()
		 */
		 ...
		 * @p [net] section.
		 *
		 * For example, this is used by @ref DarkHelp::init() when @ref DarkHelp::Config::modify_batch_and_subdivisions
		 * is enabled.
		 *
		 ...
		 * This will only be set when the driver is @ref DarkHelp::EDriver::kDarknet in @ref DarkHelp::init().
		 */
		void * darknet_net;

#if CV_VERSION_MAJOR >= 4
		/// The OpenCV network, when the driver has been set to @ref DarkHelp::EDriver::kOpenCV in @ref DarkHelp::init().
		 ...
		 * constructed), or the length of time @ref DarkHelp::predict() took to run on the last image to be processed.
		 * If using @ref DarkHelp::predict_tile(), then this will store the sum of all durations across the entire set
		 * of tiles.  @see @ref DarkHelp::duration_string()
		 */
		std::chrono::high_resolution_clock::duration duration;

		/// A copy of the most recent results after applying the neural network to an image.  This is set by @ref DarkHelp::predict().
		PredictionResults prediction_results;

		/// The most recent image handled by @ref DarkHelp::predict().
		cv::Mat original_image;

		/// The most recent output produced by @ref DarkHelp::annotate().
		cv::Mat annotated_image;

		/** The number of horizontal tiles the image was split into by @ref DarkHelp::predict_tile() prior to calling
		 * @ref DarkHelp::predict().  This is set to @p 1 if calling @ref DarkHelp::predict().  It may be &gt; 1 if calling
		 * @ref DarkHelp::predict_tile() with an image large enough to require multiple tiles.
		 *
		 * @see @ref DarkHelp::vertical_tiles
		 * @see @ref Tiling
		 * @see @ref DarkHelp::Config::enable_tiles
		 */
		size_t horizontal_tiles;

		/** The number of vertical tiles the image was split into by @ref DarkHelp::predict_tile() prior to calling
		 * @ref DarkHelp::predict().  This is set to @p 1 if calling @ref DarkHelp::predict().  It may be &gt; 1 if calling
		 * @ref DarkHelp::predict_tile() with an image large enough to require multiple tiles.
		 *
		 * @see @ref DarkHelp::horizontal_tiles
		 * @see @ref Tiling
		 * @see @ref DarkHelp::Config::enable_tiles
		 */
		size_t vertical_tiles;

		/** The size that was used for each individual tile by @ref DarkHelp::predict_tile().  This will be the size of the
		 * network when calling @ref DarkHelp::predict().
		 *
		 * For example, if the network is @p 416x416, and the image used with @ref DarkHelp::predict_tile() measures @p 1280x960, then:
		 * @li @ref DarkHelp::horizontal_tiles will be set to @p 3
		 * @li @ref DarkHelp::vertical_tiles will be set to @p 2
		 * @li @ref DarkHelp::tile_size will be set to @p "(427, 480)"
		 *
		 * @see @ref Tiling
		 * @see @ref DarkHelp::Config::enable_tiles
		 */
		 ...
		 */
		static cv::Mat resize_keeping_aspect_ratio(cv::Mat mat, const cv::Size & desired_size);
		 ...
		 */
		static cv::Mat fast_resize_ignore_aspect_ratio(const cv::Mat & mat, const cv::Size & desired_size);

		/** All of DarkHelp's configuration is stored within an instance of this class.  You can either instantiate a DarkHelp
		 * object and then access @ref DarkHelp::config to set configuration as desired, or you can intantiate a
		 * @ref DarkHelp::Config object and pass it in to the DarkHelp constructor where it will be copied.
		 *
		 * @note Some fields such as the neural network filenames are only used once when @ref DarkHelp::init() is called and
		...
		class Config final
		{
			public:
				 ...
				 * Call @ref DarkHelp::init() after changing this field to force the neural network to be re-loaded.
				 */
				...
				/** Image prediction threshold.  Defaults to @p 0.5.  @see @ref DarkHelp::predict()  @see @ref DarkHelp::annotate()
				 *
				...
				/** Used during prediction.  Defaults to @p 0.5.  @see @ref DarkHelp::predict()
				 *
				...
				/** The colours to use in @ref DarkHelp::annotate().  Defaults to @ref DarkHelp::get_default_annotation_colours().
				 *
				 * Remember that OpenCV uses BGR, not RGB.  So pure red is @p "(0, 0, 255)".
				 */
				VColours annotation_colours;

				/// Font face to use in @ref DarkHelp::annotate().  Defaults to @p cv::HersheyFonts::FONT_HERSHEY_SIMPLEX.
				cv::HersheyFonts annotation_font_face;

				/// Scaling factor used for the font in @ref DarkHelp::annotate().  Defaults to @p 0.5.
				double annotation_font_scale;

				/// Thickness of the font in @ref DarkHelp::annotate().  Defaults to @p 1.
				int annotation_font_thickness;

				/// Thickness of the lines to draw in @ref DarkHelp::annotate().  Defaults to @p 2.
				int annotation_line_thickness;

				/** If set to @p true then @ref DarkHelp::annotate() will call @ref DarkHelp::duration_string() and display on the
				 * top-left of the image the length of time @ref DarkHelp::predict() took to process the image.  Defaults to @p true.
				 *
				...
				/** If set to @p true then @ref DarkHelp::annotate() will display a timestamp on the bottom-left corner of the image.
				 * Defaults to @p false.
				 *
				 ...
				 * results (@ref DarkHelp::prediction_results) after calling @ref DarkHelp::predict() will be capped so all values
				 * are positive and do not extend beyond the edges of the image.  When set to @p false, the exact values as returned
				 * by darknet will be used.  Defaults to @p true.
				 */
				bool fix_out_of_bound_values;

				/** Determines if the predictions will be sorted the next time @ref DarkHelp::predict() is called.  When set to
				...
				/** Determines if calls to @ref DarkHelp::predict() are sent directly to Darknet, or processed first by
				 * @ref DarkHelp::predict_tile() to break the image file into smaller sections.
				 *
				 * This flag is only checked when @ref predict() is called.  If you call @ref DarkHelp::predict_tile() directly,
				 * then it bypasses the check for @p DarkHelp::Config::enable_tiles and %DarkHelp will assume that the image is a
				 ...
				 *
				 * Both @ref DarkHelp::predict() and @ref DarkHelp::predict_tile() will set the values @ref DarkHelp::tile_size,
				 * @ref DarkHelp::vertical_tiles, and @ref DarkHelp::horizontal_tiles once they have finished running.  The caller
				 * can then reference these to determine what kind of tiling was used.  Even when an image is not tiled, these
				 * variables will be set; for example, @ref DarkHelp::tile_size may be set to 1x1, and the horizontal and vertical
				 * sizes will match the neural network dimensions.
				 *
				 * The default value for @p DarkHelp::Config::enable_tiles is @p false, meaning that calling @ref DarkHelp::predict()
				...
				/** Determines which classes to suppress during the call to @ref DarkHelp::annotate().  Any prediction returned by
				 * Darknet for a class listed in this @p std::set will be ignored:  no bounding box will be drawn, and no label will
				 * be shown.  The set may be modified at any point and will take effect the next time @ref DarkHelp::annotate() is
				 * called.
				 *
				 * It is initialized by @ref DarkHelp::init() to contain any classes where the label name begins with the text
				 ...
				 * @ref DarkHelp::predict() will contain all of the objects found by Darknet, regardless of what
				...
				/** The @p driver initialization happens in @ref DarkHelp::init().  Don't change
				 * this configuration value after the @p DarkHelp object has been initialized.
				 */
				EDriver driver;
		};
	...
	protected:
		...
		/** Used by all the other @ref DarkHelp::predict() calls to do the actual network prediction.  This uses the
		 * image stored in @ref DarkHelp::original_image.
		 */
		PredictionResults predict_internal(cv::Mat mat, const float new_threshold = -1.0f);

		/// Called from @ref DarkHelp::predict_internal().  @see @ref DarkHelp::predict()
		void predict_internal_darknet();

		/// Called from @ref DarkHelp::predict_internal().  @see @ref DarkHelp::predict()
		void predict_internal_opencv();

		/** Give a consistent name to the given production result.  This gets called by both @ref DarkHelp::predict_internal()
		 * and @ref DarkHelp::predict_tile() and is intended for internal use only.
		 */
		DarkHelp & name_prediction(PredictionResult & pred);

		/// Called internally by @ref DarkHelp::predict_internal().  @see @ref DarkHelp::Config::fix_out_of_bound_values
		DarkHelp & fix_out_of_bound_normalized_rect(float & cx, float & cy, float & w, float & h);

		/// Size of the neural network, e.g., @p 416x416 or @p 608x608.  @see @ref DarkHelp::network_size()
